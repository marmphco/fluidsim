...
void main() {
    ...
    float dx = (texture2D(scalarBuffer, right).x-texture2D(scalarBuffer, left).x)*0.5;
    float dy = (texture2D(scalarBuffer, up).x-texture2D(scalarBuffer, down).x)*0.5;
    float dz = (texture2D(scalarBuffer, forward).x-texture2D(scalarBuffer, backward).x)*0.5;
